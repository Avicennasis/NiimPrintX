...
from NiimPrintX.nimmy.userconfig import _validate_dims, load_user_config, merge_label_sizes
...
def test_load_user_config_missing_file_returns_empty():
    with patch("NiimPrintX.nimmy.userconfig.CONFIG_FILE", Path("/nonexistent/path/config.toml")):
        result = load_user_config()
    assert result == {}
